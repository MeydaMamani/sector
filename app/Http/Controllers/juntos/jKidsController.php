...
use App\Exports\juntos\kids\Cred12Export;
use App\Exports\juntos\kids\CredPaqueteExport;
...
class jKidsController extends Controller
{
    ...
    public function tableResumCreds(Request $request)
    {
        ...
        if ($type == 'rn') {
            ...
            if ($anio == 'TODOS') {
                $resultPaquete = DB::connection('BD_JUNTOS')->table('dbo.CONSOLIDADO_NINO_PAQUETE_JUNTOS')
                    ->select(
                        'PROVINCIA_RES',
                        'DISTRITO_RES',
                        DB::raw("COUNT(DISTRITO_RES) DENOMINADOR"),
