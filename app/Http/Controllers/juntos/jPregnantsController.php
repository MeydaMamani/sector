...
class jPregnantsController extends Controller
{
    ...
    public function forGrafEA(Request $request)
    {
        $anio = $request->id;
        if ($anio == 'TODOS') {
            $result2M = DB::table('dbo.CONSOLIDADO_GESTANTE_PAQUETE_JUNTOS')
                        ...
                        SIFILIS_HIS IS NOT NULL AND EXAMEN_ORINA_HIS IS NOT NULL) THEN 1 ELSE 0 END) as float) / cast(COUNT(*) as float) * 100), 1) 'AVANCE_HIS'")) ->get();
    ...
    public function tableResumEA(Request $request)
    {
        $resum2 = '';
        $anio = $request->id;
        $type = $request->type;
    ...
    public function printEA(Request $request)
    {
        ...
        $a = $request->anioEA;
        $t = $request->typeEA;
        return Excel::download(new EAExport($r, $d, $a, $t), 'DEIT_PASCO REPORTE DE EXAMENES AUXILIARES.xlsx');
    }

    public function forGrafAP(Request $request)
    {
        $anio = $request->id;
        if ($anio == 'TODOS') {
            $result2M = DB::table('dbo.CONSOLIDADO_GESTANTE_PAQUETE_JUNTOS')
    ...
    public function tableResumAP(Request $request)
    {
        $resum2 = '';
        $anio = $request->id;
        $type = $request->type;
        ...
        public function printAP(Request $request)
        {
                    ...
                    $a = $request->anioAP;
                    $t = $request->typeAP;
                    return Excel::download(new APExport($r, $d, $a, $t), 'DEIT_PASCO REPORTE DE ATENCIONES PRENATALES.xlsx');
        }
