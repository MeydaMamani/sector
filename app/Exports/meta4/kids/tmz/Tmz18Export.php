...
class Tmz18Export implements FromView, ShouldAutoSize
{
    protected $red;
    protected $dist;
    protected $anio;
    ...
    public function view(): View {
        ...
        if($type == 'conteo'){
            ...
            return view('meta4.kids.tmz.tmz18.printConteo', [ 'nominal' => $rTmz18, 'anio' => $anio ]);
        }
            ...
            if($red == 'TODOS'){
                if($anio == 'TODOS'){
                    ...
                    $nominalTmz = DB::table('dbo.META4_CONSOLIDADO')
                                    ->select('PROVINCIA', 'DISTRITO', 'NUMERO_DE_DOCUMENTO_DEL_NINO', 'FECHA_DE_NACIMIENTO', 'DOSAJE_HMB_18M',
                                    DB::raw("CASE WHEN ([DOSAJE_HMB_18M] IS NOT NULL ) THEN 'Cumple' ELSE 'No Cumple' END 'TMZ_HIS'"))
                                    ->whereRaw('PERIODO = (SELECT MAX(PERIODO) FROM META4_CONSOLIDADO)')
                                    ->orderBy('PROVINCIA') ->orderBy('DISTRITO') ->get();
                }
                else{
            ...
            return view('meta4.kids.tmz.tmz18.printNominal', [ 'nominal' => $nominalTmz, 'anio' => $anio ]);
        }
    }
}
