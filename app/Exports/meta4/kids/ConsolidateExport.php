        ...
        if($r == 'TODOS'){
            ...
            if($a == 'TODOS'){
                $nominal = DB::table('dbo.META4_CONSOLIDADO')
                            ->select('*', '1CTRL_RN AS CTRL1_RN', '2CTRL_RN AS CTRL2_RN',
                                ...
                                'EH_12M', 'EH_13M', 'EH_14M', 'EH_15M', 'EH_16M', 'EH_17M',
                                'EH_18M', 'EH_19M', 'EH_20M', 'EH_21M', 'EH_22M', 'EH_23M')
                                ->whereRaw('PERIODO = (SELECT MAX(PERIODO) FROM META4_CONSOLIDADO)') ->where('PROVINCIA', $r)
                            ->orderBy('PROVINCIA') ->orderBy('DISTRITO') ->get();
            }
            else{
                $nominal = DB::table('dbo.META4_CONSOLIDADO')
                            ->select('*', '1CTRL_RN AS CTRL1_RN', '2CTRL_RN AS CTRL2_RN',
                                ...
                                'EH_18M', 'EH_19M', 'EH_20M', 'EH_21M', 'EH_22M', 'EH_23M')
                                ->whereRaw('PERIODO = (SELECT MAX(PERIODO) FROM META4_CONSOLIDADO)') ->where('PROVINCIA', $r)
                                ->whereYear('FECHA_DE_NACIMIENTO', $a)
                            ->orderBy('PROVINCIA') ->orderBy('DISTRITO') ->get();
            }
        }
        else{
            if($a == 'TODOS'){
                $nominal = DB::table('dbo.META4_CONSOLIDADO')
                                ...
                                'EH_12M', 'EH_13M', 'EH_14M', 'EH_15M', 'EH_16M', 'EH_17M',
                                'EH_18M', 'EH_19M', 'EH_20M', 'EH_21M', 'EH_22M', 'EH_23M')
                                ->whereRaw('PERIODO = (SELECT MAX(PERIODO) FROM META4_CONSOLIDADO)') ->where('DISTRITO', $d)
                            ->orderBy('PROVINCIA') ->orderBy('DISTRITO') ->get();
            }
            else{
                $nominal = DB::table('dbo.META4_CONSOLIDADO')
