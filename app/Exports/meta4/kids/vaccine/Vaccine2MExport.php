        ...
        if($type == 'conteo'){
            ...
            if($red == 'TODOS'){
                if($anio == 'TODOS'){
                    $anio = 'Todos';
                    $tVac2M = DB::table('dbo.META4_CONSOLIDADO')
                                ->select('PROVINCIA', 'DISTRITO', 'NUMERO_DE_DOCUMENTO_DEL_NINO', 'FECHA_DE_NACIMIENTO', '1_NEUMO 2M as NEUMO_2M',
                                '1_ROTA 2M as ROTA_2M','1_PENTA 2M as PENTA_2M', DB::raw("CASE WHEN ([1_NEUMO 2M] IS NOT NULL AND [1_ROTA 2M] IS NOT NULL
                                AND [1_PENTA 2M] IS NOT NULL) THEN 'Cumple' ELSE 'No Cumple' END 'CUMPLE_HIS'"))
