...
class tmz18MExport implements FromView, ShouldAutoSize
{
    protected $red;
    protected $dist;
    protected $anio;
    ...
    public function view(): View {
        ...
        if($type == 'conteo'){
            if($red == 'TODOS'){
                if ($anio == 'TODOS') {
                    $resum18 = DB::table('dbo.CONSOLIDADO_NINO_PAQUETE_JUNTOS')
                                ...
                                (DOSAJE_HEMOGLOBINA_18M IS NOT NULL) THEN 1 ELSE 0 END) as float) / cast(COUNT(*) as float) * 100), 2)
                                'AVANCE_JUNT'"), DB::raw("SUM(CASE WHEN ([DOSAJE_HMB_18M] IS NOT NULL) THEN 1 ELSE 0 END) AS TMZ_HIS"),
                                DB::raw("round((cast(SUM(CASE WHEN ([DOSAJE_HMB_18M] IS NOT NULL) THEN 1 ELSE 0 END) as float) /
            ...
            return view('juntos.kids.tmz.tmz18m.printConteo', [ 'nominal' => $resum18, 'anio' => $anio ]);
        }
        else if($type == 'nominal'){
            if($red == 'TODOS'){
                if($anio == 'TODOS'){
                    $anio = 'Todos';
                    $nomTmz18 = DB::table('dbo.CONSOLIDADO_NINO_PAQUETE_JUNTOS')
                                ->select('PROVINCIA_RES', 'DISTRITO_RES', 'DNI_MO', 'FECHA_DE_NAC_MO', 'DOSAJE_HEMOGLOBINA_18M',
                                DB::raw("(CASE WHEN (DOSAJE_HEMOGLOBINA_18M IS NOT NULL) THEN 'Cumple' ELSE 'No Cumple' END) AS TMZ_JUNT"), 'DOSAJE_HMB_18M',
                                DB::raw("(CASE WHEN ([DOSAJE_HMB_18M] IS NOT NULL) THEN 'Cumple' ELSE 'No Cumple' END) AS TMZ_HIS")) ->orderBy('PROVINCIA_RES')
                    ...
                    $nomTmz18 = DB::table('dbo.CONSOLIDADO_NINO_PAQUETE_JUNTOS')
                                ->select('PROVINCIA_RES', 'DISTRITO_RES', 'DNI_MO', 'FECHA_DE_NAC_MO', 'DOSAJE_HEMOGLOBINA_18M',
                                DB::raw("(CASE WHEN (DOSAJE_HEMOGLOBINA_18M IS NOT NULL) THEN 'Cumple' ELSE 'No Cumple' END) AS TMZ_JUNT"), 'DOSAJE_HMB_18M',
                                DB::raw("(CASE WHEN ([DOSAJE_HMB_18M] IS NOT NULL) THEN 'Cumple' ELSE 'No Cumple' END) AS TMZ_HIS"))
                ...
                if($anio == 'TODOS'){
                    ...
                    $nomTmz18 = DB::table('dbo.CONSOLIDADO_NINO_PAQUETE_JUNTOS')
                                ->select('PROVINCIA_RES', 'DISTRITO_RES', 'DNI_MO', 'FECHA_DE_NAC_MO', 'DOSAJE_HEMOGLOBINA_18M',
                                DB::raw("(CASE WHEN (DOSAJE_HEMOGLOBINA_18M IS NOT NULL) THEN 'Cumple' ELSE 'No Cumple' END) AS TMZ_JUNT"), 'DOSAJE_HMB_18M',
                                DB::raw("(CASE WHEN ([DOSAJE_HMB_18M] IS NOT NULL) THEN 'Cumple' ELSE 'No Cumple' END) AS TMZ_HIS")) ->where('PROVINCIA_RES', $red)
                                ->whereYear('FECHA_DE_NAC_MO', $anio)->orderBy('PROVINCIA_RES') ->orderBy('DISTRITO_RES') ->get();
                }
            }
            else if($dist != 'TODOS'){
                if($anio == 'TODOS'){
                    ...
                    $nomTmz18 = DB::table('dbo.CONSOLIDADO_NINO_PAQUETE_JUNTOS')
                                ->select('PROVINCIA_RES', 'DISTRITO_RES', 'DNI_MO', 'FECHA_DE_NAC_MO', 'DOSAJE_HEMOGLOBINA_18M',
                                DB::raw("(CASE WHEN (DOSAJE_HEMOGLOBINA_18M IS NOT NULL) THEN 'Cumple' ELSE 'No Cumple' END) AS TMZ_JUNT"), 'DOSAJE_HMB_18M',
                                DB::raw("(CASE WHEN ([DOSAJE_HMB_18M] IS NOT NULL) THEN 'Cumple' ELSE 'No Cumple' END) AS TMZ_HIS")) ->where('DISTRITO_RES', $dist)
                                ->whereYear('FECHA_DE_NAC_MO', $anio)->orderBy('PROVINCIA_RES') ->orderBy('DISTRITO_RES') ->get();
                }
            }

            return view('juntos.kids.tmz.tmz18m.printNominal', [ 'nominal' => $nomTmz18, 'anio' => $anio ]);
        }
    }
}
