...
class APExport implements FromView, ShouldAutoSize
{
    ...
    public function view(): View {
        ...
        if($type == 'conteo'){
            ...
            return view('juntos.pregnants.AP.printConteo', [ 'nominal' => $resum2 ]);
        }
                ...
                $nominal = DB::table('dbo.CONSOLIDADO_GESTANTE_PAQUETE_JUNTOS')
                        ->select('PROVINCIA_RES', 'DISTRITO_RES', 'DNI_MO', 'CONTROL1', DB::raw("Convert(Integer, Datediff(Day, [CONTROL1], Getdate())/30) EDADMESES"),
                            ...
                            DB::raw(" CASE
                                ...
                                ELSE 'No cumple'
                            END 'CUMPLE_HIS'")) ->where('PROVINCIA_RES', $red)
                        ->orderBy('PROVINCIA_RES') ->orderBy('DISTRITO_RES') ->get();
            }
            else if($dist != 'TODOS'){
                $nominal = DB::table('dbo.CONSOLIDADO_GESTANTE_PAQUETE_JUNTOS')
                        ->select('PROVINCIA_RES', 'DISTRITO_RES', 'DNI_MO', 'CONTROL1', DB::raw("Convert(Integer, Datediff(Day, [CONTROL1], Getdate())/30) EDADMESES"),
                            ...
                            DB::raw(" CASE
                                ...
                                ELSE 'No cumple'
                            END 'CUMPLE_HIS'")) ->where('DISTRITO_RES', $dist)
                        ->orderBy('PROVINCIA_RES') ->orderBy('DISTRITO_RES') ->get();
            }

            return view('juntos.pregnants.AP.printNominal', [ 'nominal' => $nominal ]);
        }
    }
